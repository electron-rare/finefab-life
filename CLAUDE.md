...
| Indexeur Nextcloud → Qdrant | `nc-rag-indexer/` → voir `nc-rag-indexer/CLAUDE.md` |
...
life-web (React/nginx :80)
  → life-reborn (Hono :3210)
    → life-core (FastAPI :8000)
       ↕  qdrant :6333  redis :6379  langfuse :3000
       ↕  vllm-kxkm-proxy (socat host:11436 → KXKM-AI:8000)
       ↕  ollama-cils-proxy (socat host:11435 → Cils:11434)
...
- `life-internal` — communication inter-services (tous sauf life-web)
- `traefik` — routage public via Traefik **local** (Let's Encrypt dnsChallenge Cloudflare)
- socat containers en mode `network_mode: host` pour atteindre machines distantes via Tailscale
- life-core atteint les LLM via `host.docker.internal:11435` (Ollama/Cils) et `host.docker.internal:11436` (vLLM/KXKM-AI)
